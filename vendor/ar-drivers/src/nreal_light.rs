...
pub struct NrealLightSlamCamera {
    ...
    /// Bytes read from the bulk endpoint but not yet consumed as a frame.
    /// A single `read_bulk` may return more than one frame's worth of data
    /// (frame + start of the next), so the tail is carried over instead of
    /// discarded — discarding it forced a second blocking read (~40 ms)
    /// every frame, a persistent ~13 fps state.
    pending: Vec<u8>,
}
...
impl NrealLightSlamCamera {
    ...
    fn new_common(device_handle: rusb::DeviceHandle<rusb::GlobalContext>) -> Result<Self> {
        ...
        let result = Self {
            device_handle,
            pending: Vec::with_capacity(615908 * 2),
        };
        Ok(result)
    }
    ...
    ///
    /// A single bulk read is NOT guaranteed to be exactly one 615908-byte
    /// frame: it may return 0 bytes (right after stream start), exactly one
    /// frame, or more than one frame's worth when the kernel USB buffer has
    /// accumulated bytes (frame + start of next, ending on a short packet).
    /// Upstream discarded anything that was not exactly one frame, forcing a
    /// second blocking read (~40 ms) per frame — a persistent ~13 fps state.
    /// Instead, reads accumulate into `pending` and exactly one frame is
    /// extracted; the tail (start of the next frame) is carried over.
    pub fn get_frame(&mut self, timeout: Duration) -> Result<NrealLightSlamCameraFrame> {
        const FRAME_SIZE: usize = 615908;
        let started = std::time::Instant::now();
        loop {
            if self.pending.len() >= FRAME_SIZE && self.pending[0] != 0 {
                break;
            }
            // A frame always starts with a non-zero header byte (0x0c).
            // If the buffer starts with 0 we are misaligned — drop one byte
            // and keep reading (defensive; reads normally end on short
            // packets at frame boundaries, so this should not trigger).
            if self.pending.first() == Some(&0) {
            ...
            let mut chunk = vec![0u8; FRAME_SIZE * 2];
            let recvd = self.device_handle.read_bulk(0x81, &mut chunk, actual_timeout)?;
            if std::env::var_os("AR_DRIVERS_DEBUG_READS").is_some() {
                eprintln!(
                    "[get_frame] read recvd={recvd} pending={} elapsed_ms={:.1}",
                    self.pending.len(),
                    started.elapsed().as_secs_f64() * 1000.0
                );
            }
            self.pending.extend_from_slice(&chunk[..recvd]);
        }

        // `pending` now holds at least one complete frame starting at index 0
        // (reads end on short packets at frame boundaries, so the buffer is
        // frame-aligned). Take the first frame, keep the tail for next call.
        let tail = self.pending.split_off(FRAME_SIZE);
        let mut bulk_data = std::mem::replace(&mut self.pending, tail);
